...
test('two parameters', t => {
  ...
  t.is(paramManager.getParam('Currency').value, drachmaBrand);
  t.deepEqual(
    paramManager.getParam('Amt').value,
...
test('Amount', async t => {
  ...
  const paramManager = makeParamManagerBuilder()
    ...
    .build();
  t.deepEqual(
    paramManager.getParam('Shimmer').value,
    AmountMath.make(brand, 250n),
  );
...
test('params one installation', async t => {
  ...
  t.deepEqual(paramManager.getParam(installationKey).value, installationHandle);
  t.throws(
  ...
  t.deepEqual(paramManager.getParam(installationKey).value, handle2);
...
test('params one instance', async t => {
  ...
  t.deepEqual(paramManager.getParam(instanceKey).value, instanceHandle);
  t.throws(
  ...
  t.deepEqual(paramManager.getParam(instanceKey).value, handle2);
...
test('Invitation', async t => {
  ...
  t.is(paramManager.getParam('Currency').value, drachmaBrand);
  t.is(paramManager.getParam('Amt').value, drachmaAmount);
  const invitationActualAmount = paramManager.getParam('Invite').value;
  t.is(invitationActualAmount, invitationAmount);
  // @ts-ignore invitationActualAmount's type is unknown
  t.is(invitationActualAmount.value[0].description, 'simple');
...
test('two Nats', async t => {
  ...
  t.is(paramManager.getParam('Acres').value, 50n);
  t.is(paramManager.getParam('SpeedLimit').value, 299_792_458n);
...
test('Ratio', async t => {
  ...
  const paramManager = makeParamManagerBuilder()
    ...
    .build();
  t.is(paramManager.getParam('GoldenRatio').value, ratio);
  ...
  t.is(paramManager.getParam('GoldenRatio').value, morePrecise);
  // @ts-ignore updateGoldenRatio is a generated name
...
test('Strings', async t => {
  const paramManager = makeParamManagerBuilder()
    ...
    .build();
  t.is(paramManager.getParam('OurWeapons').value, 'fear');

  // @ts-ignore updateOurWeapons is a generated name
  paramManager.updateOurWeapons('fear,surprise');
  t.is(paramManager.getParam('OurWeapons').value, 'fear,surprise');
  // @ts-ignore updateOurWeapons is a generated name
...
test('Unknown', async t => {
  const paramManager = makeParamManagerBuilder()
    ...
    .build();
  t.is(paramManager.getParam('Surprise').value, 'party');

  // @ts-ignore updateSurprise is a generated name
  paramManager.updateSurprise('gift');
  t.is(paramManager.getParam('Surprise').value, 'gift');
  // @ts-ignore updateSurprise is a generated name
  paramManager.updateSurprise(['gift', 'party']);
  t.deepEqual(paramManager.getParam('Surprise').value, ['gift', 'party']);
});
