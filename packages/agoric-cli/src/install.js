...
export default async function installMain(progname, rawArgs, powers, opts) {
  ...
  const updateSdkVersion = async forceVersion => {
            ...
            for (const pkg of Object.keys(deps)) {
              if (versions.has(pkg)) {
                if (deps[pkg] === forceVersion) {
                  // We need to remove the old package and install to prune the
                  // old dependencies.  Only after that can we add the new deps
                  // and run install to pick them up.
    ...
    if (pruningTodo.length) {
      // Run all the pruning tasks in parallel.
    ...
    // Ensure we do all the package.json updates after any necessary pruning,
